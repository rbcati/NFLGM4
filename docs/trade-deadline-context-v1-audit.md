...
- **Valuation evidence:** the trade engine deliberately defaults absent talent inputs for resilience, so V1 requires a recorded, finite, positive, canonical-scale top-level `ovr` before calling that authority for display. Blank, zero, non-finite, missing, or out-of-range OVR suppresses the candidate instead of exposing a fallback-derived value. `potential` remains optional because the canonical mod player schema declares it optional (`src/types/modding.ts`) and the valuation authority intentionally uses recorded OVR when it is absent; V1 therefore does not reject otherwise valid imported active players solely for missing potential.
...
- **Standings authority:** `prepareStandingsView()` in `src/views/standingsView.js`. V1 uses its division ordering only in regular season and playoffs, where the view represents current-season standings. Preseason may intentionally retain archived prior-season rows, so V1 omits division position then rather than labeling the archived finish as current. Record and streak come directly from the supplied user team. It does not claim standings movement because no before/after snapshot is supplied.
...
A player is evaluated once per unique canonical ID after resolving supplied roster IDs against `league.players`. The player must belong to the supplied user team, be an active-roster or injured-reserve player according to Player Decision Presentation, carry recorded valid OVR evidence for displayed valuation, and not be a free agent, practice-squad player, prospect/draft-class member, retired player, foreign-team player, or stale reference.
...
Legacy saves inherit the canonical normalized deadline default. Missing team, roster reference, standings, contract term, role, value inputs, or age only removes context/candidates. Preseason archived standings are not presented as a current division position. Numeric and string IDs compare canonically. No missing field becomes zero, expiring, depth, urgency, interest, or demand.
