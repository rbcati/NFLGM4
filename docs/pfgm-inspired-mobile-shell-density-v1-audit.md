...
- **Rows/cards:** shared screen stacks, row stacks, compact list rows, section cards, team alerts, standings mini rows, News rows, and weekly result cards use tighter gaps, charcoal bands, and visible dividers. Existing content and click semantics remain unchanged.
...
- **Bottom navigation:** destinations, labels, callbacks, badges, drawer, collapse behavior, and accessible names are unchanged. Chrome is now full-width and flat; active tabs have an icon/color treatment, top indicator, bold label, and `aria-current="page"`.
...
- The shell uses full-width sizing with `max-width: 100%`, contained scrolling for tab/filter strips, `min-width: 0` on screen stacks, and document-level horizontal clipping as a final paint guard. Interactive tab and nav targets remain at least 44px high.
...
- Selected tabs do not rely on color alone: they use an underline, font weight, and text contrast. Bottom-nav selection adds a top indicator, bold label, and `aria-current`. Focus uses a two-pixel green outline. Heading elements and levels were not changed.
...
- Targeted Vitest command covering MobileNav, shell navigation, SaveSlotManager, LeagueHub, TeamHub, NewsFeed, FranchiseHQ/mobile shell, and LeagueDashboard player-profile navigation: 9 files and 82 tests passed.
- `npm run test:unit`: 472 files and 5,869 tests passed.
...
- `npx playwright test tests/e2e/mobile_navigation_integrity.spec.js`: not executed to an assertion result because the Chromium executable was absent.
