...
function buildPlayoffIndex(seasonRow) {
  const index = new Map();
  const rounds = seasonRow?.playoffBracketSnapshot?.rounds;
  if (!Array.isArray(rounds)) return index;
  for (const round of rounds) {
    for (const game of round?.games ?? []) {
      const gameId = game?.gameId ?? game?.id;
      if (gameId == null || gameId === '') continue;
      index.set(String(gameId), round?.label ?? null);
    }
  }
  return index;
}
...
function normalizeMeeting(game, context) {
  ...
  const explicitlyPostseason = Boolean(context?.playoffStage || isPostseasonGame({ ...game, stage: stageToken }));
...
function candidateRank(candidate) {
  return (candidate?._source === 'current_schedule' ? 100 : 0)
