...
function pickUnitPlayers(
  ...
  for (const pos of priority) {
    const slice = roster.filter((player) => {
      return getPlayerScrimmageUnitRow(player, group)?.key === pos;
  ...
  if (picked.length < targetSize) {
    ...
    const hasQuarterback = group === 'OFFENSE'
      && picked.some((player) => getPlayerScrimmageUnitRow(player, group)?.key === 'QB');
    const fillers = roster
      .filter((player) => !pickedIds.has(String(player.id)))
      .filter((player) => !hasQuarterback || getPlayerScrimmageUnitRow(player, group)?.key !== 'QB')
      .sort((a, b) => stablePlayerSort(a, b, group))
